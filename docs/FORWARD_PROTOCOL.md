...
| 跟踪误差可计天数 | ≥ 5 日 | 排除两类「重放无法复现」的日期（见下）后的有效样本量 | 判失败 |
...
**跟踪误差的两类排除日**（`forward_health.py` 自动处理，并在工件里列出）：

1. **live 成交日**（`fills.source = "live"`）：实时层在真实成交价上执行，属于外部市场事件，
   分钟 bar 重放无法复现（实时层在 live 日期本来就被门控关闭）；
2. **配置生效日之前**（`forward.config_freeze_date`，当前 `2026-09-09`）：那些账本行是
   旧配置产出的（例如 2.5% 止损宽度），用今天的配置重放必然不同。

2026-09-09 的试运行正是这个情况：09-01→09-09 的重放差 0.99pp/日，逐日拆解后发现
09-01 的 `000596.SZ` 在旧 2.5% 宽度下触发「冲高离场」（`3 × 0.025` 阈值 98.6 < 100.88），
而在新 3.5% 宽度下阈值升到 101.36 > 100.88，不再触发 —— 差异来自**配置变更**而非数据漂移
或管线缺陷。排除配置生效日前的日期后，该窗口只剩 1 个有效日（< 5 日下限），闸门正确地
判为**未测量（失败）**；真正的前向窗口从 2026-09-10 起算，全部由当前配置产出。
...
**实测（生产库 6 只标的 / 22,928 根 K 线，其中 21,901 根为复权事件行）**：

| 列 | 推断基准 | 相对 `raw_close` 中位误差 | 相对 `raw_close × factor` 中位误差 |
| --- | --- | --- | --- |
| `open` | **raw** | 9.3e-3 | 3.1e-1 |
| `high` | **raw** | 1.0e-2 | 3.2e-1 |
| `low` | **raw** | 9.6e-3 | 2.9e-1 |
| `close` | **adjusted** | 2.4e-1 | 1.3e-6 |

（`~1e-2` 残差不是噪声，是当日真实振幅。）读取层把该报告挂到 `market.basis`，
任何消费者都能记录自己的数字属于哪一支。
...
**实测（2026-09-09，生产库）**：`data_as_of=2026-09-09`、5,164 只标的 / 12,535,534 行；
**2,651 只标的**的末根 bar 因子 ≠ 1（全部 < 1），最小 `0.4563`（`000793.SZ`）；
`factors_sha256 = 9011534279…fed8db` 在三次独立抓取中完全一致；
基线 vs 当前 `--compare`：`common=5164 new=0 lost=0, drifted=0/5164 → stable`（退出码 0）。
...
`outputs/forward/forward_health_shakedown_20260909.json`（**非**前向窗口，仅验证管线）：

| 硬闸门 | 结果 | 说明 |
| --- | --- | --- |
| 跟踪误差 | **未测量 → FAIL** | 仅 1 个有效日（5 个配置生效日前的日期被排除），低于 5 日下限 |
| 违规计数 | PASS | T+1/整手/最小变动价位/涨跌停/停牌 全部为 0 |
| 费用偏差 | PASS | 计费 vs 预注册规格 = 0.0% |
| 成交价完整性 | PASS | 0.0 bp（1 笔拍卖单 vs 当日收盘价；5 笔 replay 单按设计跳过） |
| 数据新鲜度 | PASS | 0 天 |
| 符号级分钟覆盖 | PASS | 100%（800 只） |
| 实时层可用率 | **未测量 → FAIL** | 心跳文件自 2026-09-09 起才有，此窗口无记录 |

这正是预期的行为：**闸门在证据不足时拒绝通过**，而不是把 0.99pp/日 的重放差异
（其成因为配置变更，见 §1.3）读成「管线有问题」。
