"""A-track follow-up grid: short leg on/off x rebalance 10/20 days.

Full-panel 2026 replay (same setup as dual_track_tune). Given B's result —
the short leg bled through the 2026 uptrend — check whether A (2M) also wins
long-only.
"""
...
VARIANTS = [
    {"label": "A_10_10_rb10", "cash": 2_000_000, "long_pct": 0.10, "short_pct": 0.10,
     "rebalance_days": 10, "notional_floor": 0, "band_frac": 0.0},
    {"label": "A_10_0_rb10", "cash": 2_000_000, "long_pct": 0.10, "short_pct": 0.0,
     "rebalance_days": 10, "notional_floor": 0, "band_frac": 0.0},
    {"label": "A_10_0_rb20", "cash": 2_000_000, "long_pct": 0.10, "short_pct": 0.0,
     "rebalance_days": 20, "notional_floor": 0, "band_frac": 0.0},
    {"label": "A_15_0_rb10", "cash": 2_000_000, "long_pct": 0.15, "short_pct": 0.0,
     "rebalance_days": 10, "notional_floor": 0, "band_frac": 0.0},
]
...
def main() -> int:
    ...
    for v in VARIANTS:
        ...
        ledger_path = ROOT / "outputs" / f"_atune_{v['label']}.sqlite"
        ...
        print(f"{v['label']:14s} cum={results[v['label']]['cum_return']:+.2%} "
    ...
    out_path = ROOT / "outputs" / "a_track_grid.json"
