...
1. **全市场入库恢复**：4,364 只标的在 2025-12-31 后停更，全市场/成分研究已过期；
   或明确把研究范围收敛到 D 池。
2. ~~**用 800 只真实股票池复跑止损宽度 A/B 与网格**~~（**已完成 2026-09-10**）：8 变体 ×
   两窗口重跑于修正后的 800 只池，`outputs/d_stop_grid_{is_2026,oos_2025h2}_800.json`；
   部署仍是 `flat_3p5`（跨窗口 max-min 0.40，8 只变体最高），前向候选换成 `atr_1p0_25_35`
   （max-min 0.29）。**排序在两个窗口之间不一致**（Spearman ≈ −0.19），所以单窗口最优
   **不得**当作改进依据——见 `docs/D_TRACK_EVIDENCE.md` §三 与 §九。
3. **前向积累样本**：干净窗口自 2026-09-11 起算（开窗前记录 v8 已冻结并绑定 policy +
   代码指纹）；每日 15:35 追加一行 `outputs/forward/forward_samples.jsonl`，只有
   `convention_ok=true` 的日子计入干净样本。实时执行样本要从 1 笔积累到至少几十笔。
4. **影子盘历史曲线的口径**（①）：已按仓库 era 切成 4 段并逐段重放，**只有段 #1**
   （2026-01-05→08-28，163 日，+20.93% / 251 笔，旧 2.5% 止损、301 只截面）可以引用为
   「当时口径下的历史」；整条 168 日曲线（+24.58%）**混口径**，不得当作单一策略业绩。
   `scripts/shadow_series.py` 的**切片起点敏感**问题（D-14）仍未定位，工件已标
   `publishable: false`。
5. **偏差压力测试（D9，阻断）**：截面构成通道 9.60pp/年 = 1.2× alpha，未降到
   0.3× α（2.4pp）以下前不得开启自进化闭环。退市通道 1.75pp 单独看不阻断，
   但**判定只取最差通道**；另有「指数成分前瞻」通道因缺历史成分名单**无法测量**，
   需要补齐成分股历史或明确接受 L=−80% 情形。
6. **实盘通道补齐**（C1/C2/C5/C6/C7）与**成本标定**（D10，需真实成交回填冲击成本）。
7. **文档/口径一致性**（引用前先核对，以工件与 config 为准）：
   ...
   - `LIVE_READINESS.md` 的 **D12** 仍是未勾选（引 0.376 判失败），而工件该闸门已读
     **1.00**——回补已完成，勾选状态未同步。
   - 前向冻结记录已到 **v8**（trial 5 / 14，amendments 3）：开窗前那一晚连续三次重签，
     全部由 `src/`、`scripts/`、`tests/` 的内容改动触发（含把 `panel.universe.as_of`
     从「请求日」改为「实际测量的那根 bar」）。**只要这些目录再改，就必须重签或显式豁免**；
     只改文档不受影响。现行指纹：policy `302489534115…`、code `4a7e500f…`。
