...
| 轨 | 内容 | 结论 |
|---|---|---|
| **A** GitHub 公式动物园 | alpha101×55 + gtja191×24 + alpha158×158 = **237 条**公式翻译进闭式算子库并全部可求值 | 扫描出 32 条「IC 可靠」的短周期候选，送生产风险门 → **1/32 通过**（唯一幸存 `Div(Ts_Min(Low,5), Close)`，贴门通过）。**结论：`min_lookback=60` 纪律是赚来的，不是保守**——31/32 在 IC 门上可靠，一到可交易收益的组合级门全部翻车（全样本回撤 16%–99%，多数 Sharpe 为负） |
...
| **C** ML 双轨道 | LightGBM + GPU MLP（`src/ml/`，RTX 4060），Purged-KFold/embargo + walk-forward，冻结确定性工件 + `src/ml/promote.py` 生产硬门 | 见下方「ML 对决」小节 |
...
3. **在任池在两个时段都被 4 个模型超越**，替换有充分证据；但
   ...
   §影子盘接线设计），代码中零命中——自动替换门尚未接线，生产仍是人工装配。
